...
use App\Http\lib\Valitron\src\Valitron\Validator;
class ValidadorUtil{
    ...
    public function __construct($arreglo){
        ...
        $this->validador = new Validator($arreglo);
        Validator::addRule('antes', function($field, $value,$params) {
        ...
        Validator::addRule('despues', function($field, $value,$params) {
            //Las fechas deben ya venir en formato string
            $fechaUno=DateTime::createFromFormat($params[1], $value);
            $fechaDos=DateTime::createFromFormat($params[1], $params[0]);
            if(!$fechaUno || !$fechaDos){
    ...
    public function validarTextoN($campo,$requerido,$longitudMinima=0,$longitudMaxima=0,$expresionRegular=''){
        $this->validarRequerido($campo,$requerido);
        if(!empty($longitudMinima) && !empty($longitudMaxima)){
            $this->validador->rule('lengthBetween',$campo, $longitudMinima,$longitudMaxima);
        }
